...
static int ft5406_thread(void *arg)
{
	...
	int known_ids = 0;
dev_info(&ts->pdev->dev ,"V0:touch event generated kthread ");	
	while(!kthread_should_stop())
	{

dev_info(&ts->pdev->dev ,"V1:touch event generated kthread ");
		// 60fps polling
...
dev_info(&ts->pdev->dev ,"V3:touch event generated kthread ");
			int i;
...
static int ft5406_probe(struct platform_device *pdev)
{
	...
	u32 touchbuf;
dev_info(dev, "in probe function\n ");
	
	dev_info(dev, "Probing device ft5406\n");
...
dev_info(dev," in remove function ");
	if (ts->bus_addr)
		dma_free_coherent(dev, PAGE_SIZE, ts->ts_base, ts->bus_addr);
	else if (ts->ts_base)
		iounmap(ts->ts_base);
