...
static ssize_t eeprom_ioctl(struct file* file,const int cmd, size_t count, loff_t *pos , const char *buf)
{
	printk("ioctl started\n");
	int ret=0;

	switch(cmd)
	{
	case 1:
		ret=eeprom_read(file, buf,count,pos);
	break;
	case 0:
		ret=eeprom_write(file, buf,count,pos);
	break;
	default:
		do_exit(1);
	break;
